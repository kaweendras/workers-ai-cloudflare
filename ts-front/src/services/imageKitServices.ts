...
const imagekit = new ImageKit({
    publicKey: import.meta.env.VITE_IMAGEKIT_PUBLIC_KEY || "http://localhost:4001/api/v1",
    privateKey: import.meta.env.VITE_IMAGEKIT_PRIVATE_KEY || 'your_private_key',
...
export async function getAllImagesInFolder(folder: string): Promise<any[]> {
    try {
        const result = await imagekit.listFiles({
            searchQuery: `filePath INCLUDES "${folder}"`,
            sort: 'DESC_CREATED'
        });
        return result;
    } catch (error) {
